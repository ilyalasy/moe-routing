...
def _make_causal_mask(
    ...
    mask = torch.full(
        (tgt_len, tgt_len), torch.finfo(dtype).min, device=device
    )
        ...
        if self.pretraining_tp > 1:
            ...
            if HAS_TRITON and self.use_kernel and self.hidden_act == "swiglu":
                down_proj = self.down_proj(
                    LlamaActCombine.apply(self.gate_proj(x), self.up_proj(x))
                )
            else:
                down_proj = self.down_proj(
...
class OpenMoeAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.sin, self.cos = generate_fixed_pos_embedding(
            self.head_dim, self.max_position_embeddings, 1.0, 1e4
        )
...
class OpenMoeDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, moe: bool):
        ...
        if self.moe:
            ...
            self.pre_extra_mlp_layernorm = LlamaRMSNorm(
                config.hidden_size, eps=config.rms_norm_eps
            )
        ...
        self.layers = nn.ModuleList(
            [
                OpenMoeDecoderLayer(
                    ...
                    moe=(
                        True
                        if (i + 1) % config.moe_layer_interval == 0
                        else False
                    ),
                )
...
class OpenMoeForCausalLM(OpenMoePreTrainedModel):
    ...
    def _calculate_router_loss(
        self, aux_loss: list = None, z_loss: list = None
    ):
        ...
        assert (
            len(aux_loss)
            == len(z_loss)
            == self.config.num_hidden_layers // self.config.moe_layer_interval
        )
        aux_loss = (
            self.config.router_aux_loss_factor * sum(aux_loss) / len(aux_loss)
        )
    ...
    def _calculate_loss(
        ...
        # one hot
        soft_targets = targets[..., None] == torch.arange(
            vocab_size, device=targets.device
        ).reshape((1,) * len(targets.shape) + (-1,))
        soft_targets = torch.where(
            soft_targets,
            torch.full_like(soft_targets, confidence),
            torch.full_like(soft_targets, low_confidence),
        )
        soft_targets = soft_targets.to(torch.float32)

        # cross entropy
        total_loss = ZLossCrossEntropy.apply(
            logits, soft_targets, self.config.z_loss_factor
        )
...
class ZLossCrossEntropy(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, z_loss):
        ...
        ctx.save_for_backward(
            logits, targets, exp_shifted, sum_exp, log_softmax, log_z
        )
        return loss
    ...
    def backward(ctx, *grad_outputs):
        ...
        deriv = (1 + 2 * z_loss * log_z).unsqueeze(
            -1
        ) * exp_shifted / sum_exp - targets
